"""
sync_scrapper_detail.py
====================
Script untuk menyinkronkan file-file hasil scraping dengan file master terbaru.

FUNGSI UTAMA:
  Mendeteksi perusahaan yang sudah tidak ada di file master
  (data_listing_badan_usaha_minerbaone.csv), lalu menghapus baris-baris
  terkait dari keempat file hasil scraping secara akurat dan aman.

Guide:
  1. Pastikan file master sudah diperbarui dengan daftar terbaru.
  2. Jalankan script ini.
  3. Script akan menampilkan ringkasan perubahan sebelum eksekusi.
  4. Konfirmasi dengan mengetik 'ya' untuk melanjutkan.

MODE DRY-RUN:
  Ubah DRY_RUN = True untuk melihat perubahan yang akan terjadi
  tanpa benar-benar mengubah file apapun.
...
#Config
DRY_RUN = False   # True = hanya tampilkan perubahan tanpa eksekusi
...
# Semua file hasil scraping memakai kolom ini sebagai foreign key ke master
KOLOM_KUNCI = "Nama Perusahaan Asal"
...
def buat_backup(daftar_file):
    """
    Buat salinan backup semua file hasil scraping sebelum dimodifikasi.
    Backup disimpan di FOLDER_BACKUP dengan timestamp di nama file.
    """
...
def baca_csv_aman(path_file):
    """
    Baca CSV dengan penanganan error. Kembalikan DataFrame kosong jika gagal.
    """
...
def normalisasi_nama(series):
    """Normalisasi nama untuk perbandingan: lowercase, strip, collapse whitespace."""
    import re
...
def deteksi_perusahaan_hilang(df_master, df_info):
    """
    Bandingkan daftar perusahaan di master vs di FILE_INFO.
    Kembalikan set nama perusahaan yang ada di FILE_INFO tapi
    sudah tidak ada di master (artinya harus dihapus dari semua file).
    """
...
# ─────────────────────────────────────────────
# HAPUS BARIS DARI SATU FILE
# ─────────────────────────────────────────────
def hapus_dari_file(path_file, perusahaan_hilang, dry_run=False):
    """
    Hapus semua baris yang kolom KOLOM_KUNCI-nya ada di perusahaan_hilang.

    Parameter:
      path_file         : path ke file CSV yang akan dimodifikasi
      perusahaan_hilang : set nama perusahaan (string asli, bukan normalized)
      dry_run           : jika True, hanya hitung dan tampilkan tanpa menghapus

    Kembalikan dict dengan ringkasan: jumlah baris sebelum, sesudah, dihapus.
    """
...
def main():
    ...
    # Menghitung dampak per file (dry-run dulu untuk preview) ────────────
